...
- **Release notes policy (mandatory):** GitHub Release title and body must be Chinese for end users. Title format: `Cursor 中文增强包 v<version>`. Body is generated by `scripts/generate-release-notes.ps1` and validated by `scripts/tool/validate-release-notes-cli.js` before publish. When cutting a release, add Chinese highlights for the version in `$versionHighlights` inside `generate-release-notes.ps1`. Do not publish empty or English-only release notes.
