...
| 11B shadow/canary/enforced rollout | completed | shadow default + canary/legacy CLI gates + validateRolloutPromotion + release.yml evidence gate + docs; Critical/Important fix: canary empty-artifacts → shadow-style legacy; release blocks missing/incomplete evidence (no forged seed, `--require-promotable`); unmeasured gates no longer default pass; unit 16/16 + canary/rollout integration GREEN |
| Final merge Criticals (admission/shards/evidence) | completed | Wire real prepare admission (units+surfaces→outcomes+proofKey→BLOCKED fail-closed); workbench compiles/returns runtimeShards for manifest/verify size evidence; release.yml shadow-apply persists rollout-evidence (no verify-only demand / no forged seeds; `--require-promotable` still blocks incomplete). Deferred Important: synthetic drift matrix, canary transactional writes, validateTranslationUnits owner fields. |
