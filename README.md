...
- **已验证版本（推荐）**：
  - Cursor **`3.10.17`**（当前维护与测试基准）
  - VS Code 内核 `1.125.0`
  - 官方中文语言包（与内置 VS Code 版本匹配，安装时自动校验）
- **建议**：请优先使用**已验证的最新 Cursor 版本**安装本工具，以获得更完整的汉化覆盖与更稳定的运行时表现；过旧版本可能缺少 Glass / 插件市场等新界面锚点，部分文案仍会显示英文。
- 其他版本通常也可尝试，安装后请运行 `doctor.ps1` 验证兼容性；若 `verify` 报告静态 patch 未命中，说明该 Cursor 版本尚未纳入维护范围。
...
- 覆盖设置页、Glass 聊天 / Agent 工具行、插件市场壳层与描述懒加载等界面
- 插件市场描述默认走本地 catalog 懒加载翻译；关闭在线远程翻译以减轻性能开销
