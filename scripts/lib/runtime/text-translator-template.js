const { serializeMappingsCompact, generateDeserializerCode } = require('./compact-serialization');

function serializeMappings(mappings) {
  return serializeMappingsCompact(mappings);
}
...
function buildRuntimeHeader({
  ...
  return [
    ...
    `  let translationMappings = ${serializeMappings(generalRuntimeMappings)};`,
    `  let inlineTranslationMappings = ${serializeMappings(inlineTextMappings)};`,
    `  let productTipMappings = ${serializeMappings(scopedProductTipMappings)};`,
    ...
    '        let regex;',
    '        try { regex = new RegExp(entry.originalText, entry.flags || "g"); } catch { continue; }',
    '        current = current.replace(regex, entry.changeText);',
    '      }',
    '    }',
    '    return current;',
    '  }',
    '  function __cursorZhTranslateInlineText(text) {',
    '    if (typeof text !== "string" || text.length === 0) return text;',
    '    const pools = [inlineTranslationMappings, translationMappings];',
    '    for (let p = 0; p < pools.length; p++) {',
    '      const mappings = pools[p];',
    '      for (let i = 0; i < mappings.length; i++) {',
    '        const entry = mappings[i];',
    '        if (!entry || !entry.originalText) continue;',
    '        if (entry.searchType === "exact" && text.trim() === entry.originalText) return entry.changeText;',
    '      }',
    '    }',
    '    return text;',
    '  }',
    '  function __cursorZhTranslateModelPickerDisplayName(text) {',
    '    if (typeof text !== "string" || text.length === 0) return text;',
    '    const exactTiers = {',
    '      High: "高",',
    '      Fast: "快速",',
    ...
    '      "Extra High": "极高",',
    '    };',
    '    if (Object.prototype.hasOwnProperty.call(exactTiers, text.trim())) {',
    '      return exactTiers[text.trim()];',
    '    }',
    '    const suffixes = [',
    '      [" Extra High", " 极高"],',
    '      [" High", " 高"],',
    ...
    '      if (text.endsWith(suffix)) return text.slice(0, -suffix.length) + suffixes[i][1];',
    '    }',
    '    return text;',
    '  }',
    ...
    '    globalThis.__cursorZhTranslateModelPickerDisplayName = __cursorZhTranslateModelPickerDisplayName;',
    '  }',
