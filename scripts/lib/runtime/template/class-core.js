function getClassCoreLines() {
  return [
    ...
    '    }',
    '    _buildEntryIndexes() {',
    '      this._scopedExactBySelector = new Map();',
    '      this._hintScopedEntries = [];',
    '      this._normalizedExactEntries = [];',
    '      this._partialEntries = [];',
    '      this._regexEntries = [];',
    '      this._globalExactEntries = [];',
    '      for (let i = 0; i < this.entries.length; i++) {',
    '        const entry = this.entries[i];',
    '        if (!entry || !entry.originalText) continue;',
    ...
    '            const selector = entry.scopeSelectors[0];',
    '            if (!this._scopedExactBySelector.has(selector)) {',
    '              this._scopedExactBySelector.set(selector, []);',
    '            }',
    '            this._scopedExactBySelector.get(selector).push(entry);',
    '          } else if (hasScopeHints) {',
    '            this._hintScopedEntries.push(entry);',
    '          } else {',
    '            this._globalExactEntries.push(entry);',
    '          }',
    '          continue;',
    '        }',
    '        if (entry.searchType === "normalizedExact") {',
    '          this._normalizedExactEntries.push(entry);',
    '          continue;',
    '        }',
    '        if (entry.searchType === "partial") {',
    '          this._partialEntries.push(entry);',
    '          continue;',
    '        }',
    '        if (entry.searchType === "regex") {',
    '          this._regexEntries.push(entry);',
    '        }',
    '      }',
    '    }',
    '    _applyMappingEntry(current, trimmed, entry) {',
    '      if (!entry || !entry.originalText) return current;',
    '      if (entry.searchType === "exact") {',
    '        if (trimmed === entry.originalText) return entry.changeText;',
    '      } else if (entry.searchType === "normalizedExact") {',
    '        if (this.normalizeText(current) === this.normalizeText(entry.originalText)) return entry.changeText;',
    '      } else if (entry.searchType === "partial") {',
    '        return current.split(entry.originalText).join(entry.changeText);',
    '      } else if (entry.searchType === "regex") {',
    '        const regex = this._getRegex(entry.originalText, entry.flags || "g");',
    '        if (regex) return current.replace(regex, entry.changeText);',
    '      }',
    '      return current;',
    '    }',
    '    _translateEntriesForElement(current, trimmed, element) {',
    '      if (element && element.nodeType === Node.ELEMENT_NODE) {',
    '        const _closestCache = new Map();',
    '        for (const [selector, entries] of this._scopedExactBySelector) {',
    '          let inScope = _closestCache.get(selector);',
    '          if (inScope === undefined) {',
    '            try { inScope = Boolean(element.closest(selector)); } catch { inScope = false; }',
    '            _closestCache.set(selector, inScope);',
    '          }',
    '          if (!inScope) continue;',
    ...
    '          }',
    '        }',
    '        const scopedLists = [this._hintScopedEntries, this._normalizedExactEntries, this._partialEntries, this._regexEntries];',
    '        for (let li = 0; li < scopedLists.length; li++) {',
    '          const list = scopedLists[li];',
    '          for (let i = 0; i < list.length; i++) {',
    '            const entry = list[i];',
    '            if (!this.matchesScope(entry, element)) continue;',
    '            current = this._applyMappingEntry(current, trimmed, entry);',
    '          }',
    '        }',
    '        return current;',
    '      }',
    '      const lists = [this._globalExactEntries, this._hintScopedEntries, this._normalizedExactEntries, this._partialEntries, this._regexEntries];',
    '      for (let li = 0; li < lists.length; li++) {',
    '        const list = lists[li];',
    '        for (let i = 0; i < list.length; i++) {',
    '          current = this._applyMappingEntry(current, trimmed, list[i]);',
    '        }',
    '      }',
    '      for (const [, entries] of this._scopedExactBySelector) {',
    '        for (let i = 0; i < entries.length; i++) {',
    '          current = this._applyMappingEntry(current, trimmed, entries[i]);',
    '        }',
    '      }',
    '      return current;',
    '    }',
    ...
    '      }',
    '      const merged = [];',
    '      this._idleContainsSet = new Set();',
    '      for (let i = 0; i < treeTasks.length; i++) {',
    '        const a = treeTasks[i];',
    '        if (!a || !a.root) continue;',
    '        if (this._idleContainsSet.has(a.root)) {',
    '          if (perf) perf.idleQueueDeduped += 1;',
    '          continue;',
    '        }',
    '        let dominated = false;',
    ...
    '          const b = merged[j];',
    '          try {',
    '            if (a.root !== b.root && a.root.closest && b.root && a.root.closest(b.root)) {',
    '              dominated = true;',
    '              break;',
    '            }',
    '          } catch {}',
    ...
    '          const b = merged[j];',
    '          try {',
    '            if (b.root !== a.root && b.root.closest && a.root && b.root.closest(a.root)) {',
    '              merged.splice(j, 1);',
    '              this._idleContainsSet.delete(b.root);',
    '            }',
    '          } catch {}',
    '        }',
    '        merged.push(a);',
    '        this._idleContainsSet.add(a.root);',
    '      }',
    ...
    '    shouldSkipElement(element) {',
      ...
      '      }',
      '    }',
    '    shouldSkipTranslatableText(text) {',
    '      if (typeof text !== "string") return false;',
    '      const trimmed = text.trim();',
    '      if (!trimmed) return false;',
    '      if (/^(Left|Right|Up|Down|PageUp|PageDown|Home|End)Arrow$/.test(trimmed)) return true;',
    '      if (/^Key[A-Z][A-Za-z0-9]*$/.test(trimmed)) return true;',
    '      return false;',
    '    }',
    ...
    '    }',
    '    shouldSyncTranslateScope(root) {',
    '      if (!root) return false;',
    '      if (root.nodeType === Node.ELEMENT_NODE && this.isScopeContainer(root)) return true;',
    '      if (!root.querySelectorAll || !Array.isArray(this.observeScopeSelectors) || this.observeScopeSelectors.length === 0) {',
    '        return false;',
    '      }',
    '      try {',
    '        return root.querySelectorAll(this.observeScopeSelectors.join(",")).length > 0;',
    '      } catch {',
    '        return false;',
    '      }',
    '    }',
    ...
    '        current = fast;',
    '      } else {',
    '        current = this._translateEntriesForElement(current, trimmed, element || null);',
    '      }',
    '      if (current === text && element && typeof __cursorZhTranslateModelPickerDisplayName === "function") {',
    '        let inModelPicker = false;',
    '        try { inModelPicker = Boolean(element.closest("[class*=\\"ui-model-picker\\"]")); } catch {}',
    '        if (inModelPicker) current = __cursorZhTranslateModelPickerDisplayName(current);',
    '      }',
    ...
    '      if (this.shouldSkipTranslatableText(text)) {',
    '        node.textContent = "";',
    '        return;',
    '      }',
