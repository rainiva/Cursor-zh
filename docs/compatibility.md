...
| 项目 | 版本 |
|---|---|
| Cursor | **`3.13.25`**（推荐；维护与 CI 测试基准） |
| VS Code 内核 | `1.128.0` |
| 官方中文语言包 | 与内置 VS Code 版本匹配（安装时自动校验） |
| 平台 | Windows |

> **建议**：请优先使用**已验证的最新 Cursor 版本**（当前为 `3.13.25`，实测 13/13 稳定锚点全部在场、apply/verify 均通过），以获得更完整的汉化与更少的英文遗漏。较旧版本（例如 `3.9.16` 及更早）不再作为维护基准，部分 embedded patch 与 Glass 锚点可能无法命中。
